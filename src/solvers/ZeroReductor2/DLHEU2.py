...
class DHEU:
    ...
    def evaluate(self,instance,override_contsol = None):
        with torch.no_grad():
            x = self.gen_x(instance).T
            if override_contsol is not None:
                x[:,5] = torch.ones(instance.n_items)*torch.mean(override_contsol)
                x[:,3] = override_contsol
            y = self.net(x)
